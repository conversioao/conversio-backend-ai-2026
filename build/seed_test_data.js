...
async function seedFullSystem() {
    ...
    try {
        ...
        // Sofia (Welcome - Day 0)
        const userSofia = await query(`INSERT INTO users (id, name, email, whatsapp, role, created_at, password_hash) 
             VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'dummy_hash') RETURNING id`, ['Sofia Teste', 'sofia@test.conversio.ai', '244999000001', 'user']);
        // Marco (Retention - Day 3)
        const userMarco = await query(`INSERT INTO users (id, name, email, whatsapp, role, created_at, password_hash) 
             VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW() - INTERVAL '3 days', 'dummy_hash') RETURNING id`, ['Marco Teste', 'marco@test.conversio.ai', '244999000002', 'user']);
        // Risk (Churn - Day 10)
        const userRisk = await query(`INSERT INTO users (id, name, email, whatsapp, role, created_at, password_hash) 
             VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW() - INTERVAL '10 days', 'dummy_hash') RETURNING id`, ['Risk Teste', 'risk@test.conversio.ai', '244999000003', 'user']);
        ...
        for (let i = 0; i < 60; i++) {
            ...
            await query(`INSERT INTO generations (user_id, type, model, status, created_at, prompt) 
                 VALUES ($1, $2, $3, $4, NOW() - INTERVAL '${daysAgo} days', $5)`, [userId, type, type === 'image' ? 'nano_pro' : 'kling', 'completed', 'Test prompt for ' + type]);
        }
        ...
        for (let i = 0; i < 15; i++) {
            ...
            await query(`INSERT INTO transactions (user_id, amount, currency, status, type, description, credits, payment_method, created_at) 
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW() - INTERVAL '${daysAgo} days')`, [userId, amount, 'Kz', 'completed', 'credit_purchase', 'Recarga de Créditos via Seeding', amount / 100, 'Multicaixa Express']);
        }
        // 5. CRM Interactions
        console.log('--- Seeding CRM Interactions ---');
        for (const userId of allUserIds) {
            await query(`INSERT INTO crm_interactions (user_id, type, content) VALUES 
                 ($1, 'signup', 'Utilizador registou-se via Landing Page.'),
                 ($1, 'conversion_hint', 'Visualizou a página de preços 2 vezes.')`, [userId]);
        }
        // 6. WhatsApp Leads (Success cases)
        console.log('--- Seeding WhatsApp Leads ---');
        await query(`INSERT INTO whatsapp_leads (phone, name, business_info, needs, status, agent_active, last_interaction) 
             VALUES 
