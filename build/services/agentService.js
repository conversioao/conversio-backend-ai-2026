...
// Funções auxiliares
// ─────────────────────────────────────────────────────────────
const personalizeMessage = (template, user) => {
...
const logNotification = async (type, title, message, icon, color, referenceId, referenceType) => {
    try {
        await query(`INSERT INTO admin_notifications (type, title, message, icon, color, reference_id, reference_type) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [type, title, message, icon, color, referenceId || null, referenceType || null]);
    }
    catch (e) {
        console.error('[Agent] Failed to log notification:', e);
    }
};
const markAgentExecuted = async (userId, agentId, message, status = 'completed') => {
    await query(`UPDATE agent_executions SET status = $1, message_sent = $2, executed_at = NOW(), whatsapp_sent = true
         WHERE user_id = $3 AND agent_id = $4`, [status, message, userId, agentId]);
...
// ─────────────────────────────────────────────────────────────
export const executeAgentForUser = async (agent, user) => {
    try {
        console.log(`[Agent] 🤖 ${agent.persona_name} (${agent.name}) → Processing user: ${user.name}`);
        // Verificar se já foi executado
        const existing = await query(`SELECT id, status FROM agent_executions WHERE user_id = $1 AND agent_id = $2`, [user.id, agent.id]);
        if (existing.rows.length > 0) {
            ...
            await query(`INSERT INTO agent_executions (user_id, agent_id, status, scheduled_at) VALUES ($1, $2, 'running', NOW())
                 ON CONFLICT (user_id, agent_id) DO UPDATE SET status = 'running'`, [user.id, agent.id]);
        }
        // ── Se requer aprovação admin, criar pedido e parar ──
        if (agent.requires_approval) {
            const approvalExists = await query(`SELECT id FROM agent_approvals WHERE user_id = $1 AND agent_id = $2 AND status = 'pending'`, [user.id, agent.id]);
            if (approvalExists.rows.length > 0) {
                console.log(`[Agent] ⏳ Approval already pending for ${user.name}`);
                return false;
            }
            const execRes = await query(`SELECT id FROM agent_executions WHERE user_id = $1 AND agent_id = $2`, [user.id, agent.id]);
            const execId = execRes.rows[0]?.id;
            await query(`INSERT INTO agent_approvals (execution_id, user_id, agent_id, type, details, status)
                 VALUES ($1, $2, $3, $4, $5, 'pending')`, [execId, user.id, agent.id, 'message_approval', {
            ...
            await query(`UPDATE agent_executions SET status = 'pending_approval' WHERE user_id = $1 AND agent_id = $2`, [user.id, agent.id]);
            // Notificação admin no painel
            await logNotification('agent_approval_required', `⚠️ ${agent.persona_name} aguarda aprovação`, `O agente ${agent.persona_name} quer enviar uma mensagem especial para ${user.name}. Aprova no painel CRM.`, '⚠️', 'yellow', user.id, 'user');
...
`.trim();
        ...
        if (user.whatsapp) {
            ...
            if (result.success) {
                ...
                await query(`INSERT INTO crm_interactions (user_id, type, content) VALUES ($1, $2, $3)`, [user.id, 'agent_message', `${agent.emoji} ${agent.persona_name}: ${finalMessage.substring(0, 100)}...`]);
                // Notificação admin
                await logNotification('agent_action', `${agent.emoji} ${agent.persona_name} agiu`, `Mensagem enviada para ${user.name} (${user.whatsapp})`, agent.emoji, 'green', user.id, 'user');
        ...
        await query(`UPDATE agent_executions SET status = 'failed' WHERE user_id = $1 AND agent_id = $2`, [user.id, agent.id]).catch(() => { });
        return false;
    }
};
// ─────────────────────────────────────────────────────────────
...
export const runWelcomeAgent = async () => {
    ...
    try {
        ...
        for (const user of users.rows) {
            ...
            await executeAgentForUser(agent, user);
        }
    }
    catch (e) {
...
// ─────────────────────────────────────────────────────────────
export const runDayNAgent = async (delayDays) => {
    ...
    try {
        ...
        for (const user of users.rows) {
            ...
            try {
                ...
                await logNotification('user_at_risk', '🚨 Utilizador em risco de churn', `${user.name} está há 10 dias sem converter. Pode precisar de intervenção manual.`, '🚨', 'red', user.id, 'user');
            }
            catch (e) { }
        }
    }
...
// ─────────────────────────────────────────────────────────────
export const approveAgentAction = async (approvalId, adminNotes) => {
    try {
        ...
        await query(`UPDATE agent_approvals SET status = 'approved', admin_notes = $1, resolved_at = NOW() WHERE id = $2`, [adminNotes || null, approvalId]);
        ...
        if (!userRes.rows[0] || !agentRes.rows[0])
            return false;
        ...
        if (result.success) {
            ...
            await query(`INSERT INTO crm_interactions (user_id, type, content) VALUES ($1, $2, $3)`, [user_id, 'agent_approved', `✅ Admin aprovou: ${agent.persona_name} → ${message.substring(0, 80)}...`]);
            await logNotification('agent_approved', `✅ Aprovação executada com sucesso`, `${agent.persona_name} enviou mensagem aprovada para ${user.name}`, '✅', 'green', user_id, 'user');
        }
...
// ─────────────────────────────────────────────────────────────
export const rejectAgentAction = async (approvalId, adminNotes) => {
    try {
        const approval = await query(`SELECT * FROM agent_approvals WHERE id = $1`, [approvalId]);
        if (!approval.rows[0])
            return false;
        const { user_id, agent_id } = approval.rows[0];
        await query(`UPDATE agent_approvals SET status = 'rejected', admin_notes = $1, resolved_at = NOW() WHERE id = $2`, [adminNotes || null, approvalId]);
        await query(`UPDATE agent_executions SET status = 'skipped' WHERE user_id = $1 AND agent_id = $2`, [user_id, agent_id]);
        return true;
    }
    catch (e) {
