        ...
        if (agent.last_run) {
            ...
            if (diffMin > 60) {
                await triggerAlert(
                    'agent_stopped', 
                    'critical', 
                    `Agente Inativo: ${agent.name}`, 
                    `O ${agent.name} não reporta actividade há ${Math.floor(diffMin)} minutos!`,
                    { agentName: agent.name, lastRun: agent.last_run }
                );
            }
        }
    }
    ...
    try {
        ...
        for (const gen of failedGens.rows) {
            ...
            await triggerAlert(
                'generation_failed',
                'warning',
                `Falha na Geração: ${gen.type.toUpperCase()}`,
                `ID: ${gen.id}\nErro: ${errorMsg}\nUser: ${gen.user_id}`,
                { generationId: gen.id, error: errorMsg }
            );
